...
function CustomerDashboard() {
  ...
  const [isFlashSaleLive, setIsFlashSaleLive] = useState(() => {
    return localStorage.getItem("flashSaleLive") === "true";
  });
  const [flashSaleExpiry, setFlashSaleExpiry] = useState(() => {
    return parseInt(localStorage.getItem("flashSaleExpiry")) || Date.now();
  });
  const [flashTimeLeft, setFlashTimeLeft] = useState({ hours: 0, minutes: 0, seconds: 0 });

  useEffect(() => {
    const handleStorageChange = () => {
      setIsFlashSaleLive(localStorage.getItem("flashSaleLive") === "true");
      setFlashSaleExpiry(parseInt(localStorage.getItem("flashSaleExpiry")) || Date.now());
    };
    window.addEventListener("storage", handleStorageChange);
    // Also poll every 10s in case it was changed in another tab without triggering storage event in same window
  ...
  useEffect(() => {
    if (!isFlashSaleLive) return;
    const updateFlashTimer = () => {
      const diff = flashSaleExpiry - Date.now();
      if (diff <= 0) {
        setIsFlashSaleLive(false);
        localStorage.removeItem("flashSaleLive");
      } else {
        const h = Math.floor(diff / (1000 * 60 * 60));
        const m = Math.floor((diff / 1000 / 60) % 60);
        const s = Math.floor((diff / 1000) % 60);
        setFlashTimeLeft({ hours: h, minutes: m, seconds: s });
      }
    };
    const timer = setInterval(updateFlashTimer, 1000);
    updateFlashTimer();
    return () => clearInterval(timer);
  }, [isFlashSaleLive, flashSaleExpiry]);

  // Order Success & Invoice Data
  ...
  // Silent products refresh that updates live catalog without resetting loading spinners
  const refreshProductsSilent = async () => {
    try {
      const prodRes = await getProducts();
      if (Array.isArray(prodRes) && prodRes.length > 0) {
        setProducts((prev) => {
          // If count changed or any new product ID detected, update immediately
          if (prodRes.length !== prev.length || prodRes.some((p) => !prev.some((x) => x.id === p.id))) {
            return prodRes;
          }
          return prev;
        });
  ...
  useEffect(() => {
    ...
    // 1. Cross-tab BroadcastChannel listener for instant live sync
    let syncChannel;
    try {
      ...
      syncChannel.onmessage = (event) => {
        if (event.data?.type === "PRODUCT_ADDED") {
          ...
          if (event.data.product?.name) {
            toast.info(`✨ Vendor published "${event.data.product.name}" live to the marketplace!`, { autoClose: 4000 });
          }
        } else if (event.data?.type === "STOCK_UPDATED") {
          refreshProductsSilent();
          const act = event.data?.action;
          if (act === "simulate_customer_purchase") {
            toast.warning("⚡ High Market Demand: Stock levels updated across sellers!", { autoClose: 3500 });
          } else if (act === "simulate_restock_cycle" || act === "quick_restock_all") {
            toast.success("📦 Inventory Update: Vendor warehouse restock complete!", { autoClose: 3500 });
          }
    ...
    // 2. Storage event listener (fires across windows / tabs)
    ...
    window.addEventListener("storage", handleStorageChange);
    window.addEventListener("productUpdated", refreshProductsSilent);

    // 3. Silent polling every 4 seconds to guarantee sync across ports/domains
    const pollTimer = setInterval(refreshProductsSilent, 4000);
    ...
    return () => {
      ...
      window.removeEventListener("storage", handleStorageChange);
      window.removeEventListener("productUpdated", refreshProductsSilent);
  ...
  const handleAddToCart = (product) => {
    if (product.stock === "No Stock" || product.stock_quantity === 0) {
  ...
  let filteredProducts = products.filter((p) => {
    ...
    if (!matchesCategory) {
      if (selectedCategory === "Mobiles") {
        matchesCategory = p.category === "Mobiles" || p.category === "Smart Mobiles" || p.category === "5G Flagships";
      } else if (selectedCategory === "Smart Mobiles") {
        matchesCategory =
          p.category === "Smart Mobiles" ||
          p.category === "5G Flagships" ||
          (p.vendor_name && p.vendor_name.toLowerCase().includes("voltx")) ||
          p.name.toLowerCase().includes("voltx");
      } else {
        ...
        <div className="header-right-actions">
          ...
          <div className="header-user-menu-wrap" ref={userMenuRef}>
            <button
              ...
              id="header-user-account-menu-btn"
            >
              <div className="header-user-avatar">
                {userName ? userName.charAt(0).toUpperCase() : "C"}
              </div>
              <div className="header-user-meta">
                <span className="header-user-name">{userName ? userName.split(" ")[0] : "Customer"}</span>
                <span className="header-user-sub">Account <FiChevronDown /></span>
              </div>
            </button>
            ...
            {isUserMenuOpen && (
              <div className="header-user-dropdown">
                <div className="user-dropdown-header">
                  ...
                  <span className={`user-dropdown-tier tier-${membershipTier.toLowerCase()}`}>
                ...
                <button
                  type="button"
                  className="user-dropdown-item logout-action"
          ...
          <button
            ...
            onClick={handleLogout}
            title="Sign Out of Customer Platform"
          ...
          <section className="marketplace-hero-section">
            ...
            <div className="lightning-deal-sidebar">
              ...
              <div className="deal-preview-card">
                <h4>{isFlashSaleLive ? "🔥 FLASH SALE IS LIVE ACROSS STORE!" : "Samsung Galaxy S24 Ultra 5G (512GB)"}</h4>
                <div className="deal-price-row">
                  {isFlashSaleLive ? (
                    <span className="deal-discount-tag" style={{ background: '#EF4444' }}>Extra 15% OFF ALL ITEMS</span>
                  ) : (
                    <>
                ...
                <span className="claim-text">
                  {isFlashSaleLive ? `Flash Sale Ends in ${flashTimeLeft.hours}h ${flashTimeLeft.minutes}m ${flashTimeLeft.seconds}s` : `92% Claimed • Ends in ${timeLeft.hours} hours`}
                </span>
          ...
          <div className="storefront-controls-bar">
            <div className="controls-left">
              ...
              <div className="price-slider-group">
                <span>Max: ₹{maxPrice >= 1000 ? `${(maxPrice / 1000).toFixed(0)}k` : maxPrice}</span>
                <input
                  type="range"
                  min="100"
                  max="150000"
            ...
            {loading ? (
              ...
              <div className="marketplace-products-grid">
                {filteredProducts.map((p) => {
                  ...
                  const isFlashItem = isFlashSaleLive;
                  const finalPrice = isFlashItem ? p.price * 0.85 : p.price;
                  const fakeOldPrice = isFlashItem ? p.price.toFixed(2) : (p.price * 1.25).toFixed(2);
                  const discountPct = isFlashItem ? "15% FLASH OFF" : "20% OFF";
                  const memberCashbackEarn = (finalPrice * cashbackRate).toFixed(2);

                  return (
                    <div key={p.id} className="amazon-product-card" style={isFlashItem ? { border: '1px solid #EF4444', boxShadow: '0 4px 15px rgba(239, 68, 68, 0.15)' } : {}}>
                      {/* Top Badges & Wishlist */}
                      <div className="card-top-row">
                        {isFlashItem ? (
                          <span className="assured-badge" style={{ background: '#EF4444', color: '#fff', border: 'none' }}>
                            <FiZap /> LIVE FLASH SALE
                          </span>
                        ) : (
                      ...
                      {/* Visual Authentic Product Image Thumbnail */}
                      <div
                      ...
                      >
                        {(() => {
                          const srcBadge = getProductSource(p);
                          return (
                            <span 
                              style={{ 
                                position: "absolute", 
                                ...
                                border: `1px solid ${srcBadge.color}35`,
                                borderRadius: "6px",
                                padding: "3px 7px",
                                fontSize: "0.68rem",
                                fontWeight: "700",
                        ...
                        <img
                          ...
                          alt={p.name}
                          className="product-grid-real-img"
                          loading="lazy"
                        />
                        {(() => {
                          const isOutOfStock = p.stock === "No Stock" || p.stock === "Out of Stock" || p.stock_quantity === 0;
                          const isLowStock = p.stock === "Low Stock" || (p.stock_quantity > 0 && p.stock_quantity <= (p.reorder_threshold || 10));
                          if (isOutOfStock) {
                            return (
                              <span className="stock-pill-badge" style={{ background: "rgba(239, 68, 68, 0.95)", color: "#FFFFFF", fontWeight: 800 }}>
                                ❌ Out of Stock
                              </span>
                            );
                          }
                          if (isLowStock) {
                            return (
                              <span className="stock-pill-badge" style={{ background: "rgba(245, 158, 11, 0.95)", color: "#FFFFFF", fontWeight: 800 }}>
                                ⚠️ Only {p.stock_quantity} Left!
                              </span>
                            );
                          }
                          return (
                            <span className="stock-pill-badge" style={{ background: "rgba(16, 185, 129, 0.9)", color: "#FFFFFF", fontWeight: 700 }}>
                              ✓ {p.stock_quantity ? `${p.stock_quantity} in stock` : "In Stock"}
                            </span>
                          );
                        })()}
                      </div>
                      ...
                      <div className="card-body">
                        ...
                        <div className="price-container">
                          <div className="main-price-line">
                            <span className="deal-price">₹{finalPrice.toFixed(2)}</span>
                            <span className="mrp-slashed">₹{fakeOldPrice}</span>
                            <span className="discount-pct" style={isFlashItem ? {color: '#EF4444', fontWeight: 800} : {}}>{discountPct}</span>
                          </div>
                        ...
                        <div className="card-action-buttons">
                          {p.stock === "No Stock" || p.stock === "Out of Stock" || p.stock_quantity === 0 ? (
                            <button
