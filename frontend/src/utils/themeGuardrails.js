...
/**
 * Returns style attributes for a container based on its page_context.
 * @param {string} pageContext - 'login' | 'customer' | 'vendor' | 'admin' | 'chairman_admin'
 */
export function getContextStyle(pageContext) {
  const theme = THEME_GUARDRAILS[pageContext] || THEME_GUARDRAILS.admin;
  return theme;
}
